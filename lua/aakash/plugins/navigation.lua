-- [[ Navigation ]]
-- Snacks handles search; Harpoon keeps deliberate pins; MiniVisits keeps history.
-- Project-oriented actions share aakash.project, including inside tool windows.
-- Picker reference: https://github.com/folke/snacks.nvim/blob/main/docs/picker.md

local project = require 'aakash.project'

local function find_project_files() Snacks.picker.files { cwd = project.get() } end
...
local function open_project_picker()
    local function select_project(picker, item, new_tab)
        ...
        vim.schedule(function()
            ...
            if project.set(item.file, true) then find_project_files() end
        end)
    end
    ...
    Snacks.picker.projects {
        ...
        recent = true,
        transform = function(item) return not item.file:find('/_archive/', 1, true) end,
        confirm = function(picker, item) select_project(picker, item, false) end,
        actions = {
            project_tab = function(picker, item) select_project(picker, item, true) end,
        },
        win = {
            input = {
                keys = {
...
local function add_symbol_ancestors(item)
    ...
    while parent and not parent.root do
        if parent.name and parent.name ~= '' then item.text = item.text .. ' ' .. parent.name end
...
return {
    ...
    -- One picker interface for files, text, help, diagnostics, and LSP results.
    {
        'folke/snacks.nvim',
        ...
        init = function()
            ...
            vim.api.nvim_create_autocmd('LspAttach', {
                ...
                callback = function(event)
                    local buf = event.buf
                    local function map(keys, picker, desc) vim.keymap.set('n', keys, picker, { buffer = buf, desc = desc }) end
        ...
        keys = {
            ...
            {
                '<leader>sw',
                function() Snacks.picker.grep_word { cwd = project.get() } end,
                mode = { 'n', 'v' },
                desc = '[S]earch current [W]ord',
            },
            {
                '<leader>sg',
                function() Snacks.picker.grep { cwd = project.get() } end,
                desc = '[S]earch by [G]rep',
            },
            {
                '<leader>sd',
                function() Snacks.picker.diagnostics { cwd = project.get(), filter = { cwd = true } } end,
    ...
    -- [[ Yank history ]]
    {
        ...
        config = function()
            ...
            vim.keymap.set({ 'n', 'x' }, '<leader>sy', function() Snacks.picker.yanky() end, {
    ...
    -- [[ Search and replace ]]
    {
        ...
        keys = {
            {
                '<leader>sR',
                function() require('grug-far').open { prefills = { paths = require('aakash.project').get() } } end,
                desc = '[S]earch and [R]eplace',
            },
            {
                '<leader>sR',
                function() require('grug-far').with_visual_selection { prefills = { paths = require('aakash.project').get() } } end,
                mode = 'v',
                desc = '[S]earch and [R]eplace',
            },
        },
