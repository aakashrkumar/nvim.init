...
return {
...

    {
        ...
        opts = function(_, opts)
            ...
            opts.formatters_by_ft.c = { 'clang-format', lsp_format = 'never' }
            opts.formatters_by_ft.cpp = { 'clang-format', lsp_format = 'never' }

            opts.formatters = opts.formatters or {}
            opts.formatters['clang-format'] = {
                -- Append to both whole-buffer and range arguments; project styles still win.
                append_args = function(_, ctx)
                    if vim.fs.root(ctx.buf, { '.clang-format', '_clang-format' }) then return {} end
                    ...
                    return {
                        ('--style={BasedOnStyle: LLVM, IndentWidth: %d, TabWidth: %d, UseTab: %s}'):format(
                            width,
                            bo.tabstop,
                            bo.expandtab and 'Never' or 'ForIndentation'
                        ),
                    }
                end,
            }
            ...
            local function format_with_style(bufnr)
                -- Only format on save when the project explicitly defines a style.
