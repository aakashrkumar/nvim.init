...
-- Searches, navigation, and task entry points share this root policy.
-- An explicitly selected tab root wins. Otherwise prefer an LSP workspace,
-- then the nearest marker, then the file's directory (or cwd for unnamed buffers).
M.patterns = {
  'Cargo.toml',
  'rust-project.json',
  'pyrightconfig.json',
...
local function buffer_path(buf) return normalize(vim.api.nvim_buf_get_name(buf)) end
